...
window.onload = function init() {
    ...
    gl.uniform1f(gl.getUniformLocation(program, "discardValue"), discardValue);
    console.log(hsv2rgb(360, 1.0, 1.0));

    window.addEventListener("keydown", function (e) {
        if (e.key === "ArrowUp" && discardValue < 4.0) {
            discardValue += 0.1;
            gl.uniform1f(gl.getUniformLocation(program, "discardValue"), discardValue);
            document.getElementById("discardValue").innerHTML = "Gildið á discardValue er: " + discardValue;
        } else if (e.key === "ArrowDown" && (discardValue > 0.0)) {
            discardValue -= 0.1;
            gl.uniform1f(gl.getUniformLocation(program, "discardValue"), discardValue);
            document.getElementById("discardValue").innerHTML = "Gildið á discardValue er: " + discardValue;
            ...
            var newDiffuse = hsv2rgb(h, 1.0, 1.0);
            lightDiffuse = vec4(newDiffuse[0], newDiffuse[1], newDiffuse[2], 1.0);
            diffuseProduct = mult(lightDiffuse, materialDiffuse);
            gl.uniform4fv(gl.getUniformLocation(program, "diffuseProduct"), flatten(diffuseProduct));
            document.getElementById("hValue").innerHTML = "Gildið á h er: " + Math.round(h);
