...
window.onload = function init() {
    ...
    window.addEventListener("keydown", function (e) {
        switch (e.keyCode) {
            ...
            case 57:
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: Ofan á húsi, horft á bílinn"
            case 37:
                speed -= 1.0;
                document.getElementById("Speed").innerHTML = "Hraði: " + Math.round(speed);
                break;
            case 38:    // up arrow
                height += 2.0;
                document.getElementById("Height").innerHTML = "Viðbótarhæð: " + height;
    ...
    canvas.addEventListener("mousemove", function (e) {
        if (movement) {
            // Calculate the difference and scale by the rotation speed
            let dY = (origX - e.offsetX) * 0.1;
            let dX = (origY - e.offsetY) * 0.1;

            // Update the spin values with damping applied
            spinY = (spinY - dX) % 360;
            spinX = (spinX - dY) % 360;
...
function render() {
    ...
    switch (view) {
        ...
        case 9:
            // Static viewpoint outside the track; camera follows car
            mv = lookAt(vec3(-40, 140.0, 18.0 + height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0));
            drawScenery(mv);
            mv = mult(mv, translate(carXPos, carYPos, 0.0));
            mv = mult(mv, rotateZ(-carDirection));
            drawCar(mv);
